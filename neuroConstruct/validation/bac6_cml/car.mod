TITLE Mod file for component: Component(id=car type=MOD_car)
...
PARAMETER {
    ...
    m_timeCourse_shift = -9.67845 
    m_timeCourse_shifth = -2.1308 
    m_timeCourse_qm = 1.15288 
    m_timeCourse_qh = 1 
    m_steadyState_TIME_SCALE = 1 (ms)
    m_steadyState_VOLT_SCALE = 1 (mV)
    m_steadyState_shift = -9.67845 
    m_steadyState_shifth = -2.1308 
    m_steadyState_qm = 1.15288 
    m_steadyState_qh = 1 
    m_q10Settings_q10Factor = 1.15288 
    ...
    h_timeCourse_shift = -9.67845 
    h_timeCourse_shifth = -2.1308 
    h_timeCourse_qm = 1.15288 
    h_timeCourse_qh = 1 
    h_steadyState_TIME_SCALE = 1 (ms)
    h_steadyState_VOLT_SCALE = 1 (mV)
    h_steadyState_shift = -9.67845 
    h_steadyState_shifth = -2.1308 
    h_steadyState_qm = 1.15288 
    h_steadyState_qh = 1 
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=car type=MOD_car), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gatesHHrates[*]/fcond, on: Component(id=car type=MOD_car), from gatesHHrates; null
    ? Path not present in component, using factor: 1
    
    fopenHHrates = 1 
    
    ? DerivedVariable is based on path: gatesHHtauInf[*]/fcond, on: Component(id=car type=MOD_car), from gatesHHtauInf; Component(id=m type=gateHHtauInf)
    ...
    ? DerivedVariable is based on path: gatesHHratesTau[*]/fcond, on: Component(id=car type=MOD_car), from gatesHHratesTau; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesTau = 1 
    
    ? DerivedVariable is based on path: gatesHHratesInf[*]/fcond, on: Component(id=car type=MOD_car), from gatesHHratesInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesInf = 1 
    
    ? DerivedVariable is based on path: gatesHHratesTauInf[*]/fcond, on: Component(id=car type=MOD_car), from gatesHHratesTauInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesTauInf = 1 
    
    ? DerivedVariable is based on path: gateHHInstantaneous[*]/fcond, on: Component(id=car type=MOD_car), from gateHHInstantaneous; null
    ? Path not present in component, using factor: 1
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = (5.5/(cosh(0.032*( m_timeCourse_V + m_timeCourse_shift +23)))) *  m_timeCourse_TIME_SCALE ? evaluable
    m_steadyState_V = v /  m_steadyState_VOLT_SCALE ? evaluable
    m_steadyState_x = 1/(1+exp(-( m_steadyState_V + m_steadyState_shift +23)/7.4)) ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=m type=gateHHtauInf), from steadyState; Component(id=null type=car_m_inf_inf)
    m_inf = m_steadyState_x ? path based
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=car_m_tau_tau)
    ...
    h_timeCourse_t = (771/(cosh(0.047*( h_timeCourse_V + h_timeCourse_shifth +79)))) *  h_timeCourse_TIME_SCALE ? evaluable
    h_steadyState_V = v /  h_steadyState_VOLT_SCALE ? evaluable
    h_steadyState_x = 1/(1+exp(( h_steadyState_V + h_steadyState_shifth +79)/7.8)) ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=h type=gateHHtauInf), from steadyState; Component(id=null type=car_h_inf_inf)
    h_inf = h_steadyState_x ? path based
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=car_h_tau_tau)
